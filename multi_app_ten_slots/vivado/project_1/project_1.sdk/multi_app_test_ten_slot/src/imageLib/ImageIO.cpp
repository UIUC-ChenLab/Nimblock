...
void read_header(FIL *fp, const char *imtype, char c1, char c2,
                 int *width, int *height, int *nbands, int thirdArg)
{
	...
	if (my_getc(fp) != c1 || my_getc(fp) != c2)
...
{
    // Open the file and read the header
    //FILE *stream = f_open(filename, "rb");
	FIL stream;
	FRESULT rc;
	rc = f_open(&stream, filename, FA_READ);
	if (rc != 0) {
    ...
    if (isGray || isFloat) { // read PGM or PMF
		...
		for (int y = 0; y<sh.height; y++) {
    	    ...
    	    //if ((int)f_read(ptr, sizeof(uchar), n, stream) != n)
			unsigned int bytes_read;
			f_read(&stream, ptr, sizeof(uchar)*n, &bytes_read);
			if (bytes_read != sizeof(uchar)*n)
		...
		rowBuf.resize(n);
		for (int y = 0; y<sh.height; y++) {
	   	    //if ((int)f_read(&rowBuf[0], sizeof(uchar), n, stream) != n)
			unsigned int bytes_read;
			f_read(&stream, &rowBuf[0], sizeof(uchar)*n, &bytes_read);
			if (bytes_read != sizeof(uchar)*n)
    	        throw CError("ReadFilePGM(%s): file is too short", filename);
