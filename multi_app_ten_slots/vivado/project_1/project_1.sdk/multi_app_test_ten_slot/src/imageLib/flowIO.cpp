...
//	(int)fread(&height, sizeof(int),   1, stream) != 1)
    unsigned int tag_len, width_len, height_len;
    tag_len = f_read(&stream, &tag, sizeof(float), &tag_len);
    width_len = f_read(&stream, &width, sizeof(int), &width_len);
    height_len = f_read(&stream, &height, sizeof(int), &height_len);
//    xil_printf("%d\n\r", tag_len);
//    xil_printf("%d\n\r", width_len);
//    xil_printf("%d\n\r", height_len);
//    xil_printf("%f\n\r", tag);
...
//    xil_printf("%d\n\r", height);
//
//    if (tag_len != sizeof(float) ||
//    	width_len != sizeof(int) ||
//		height_len != sizeof(int))
//    	throw CError("ReadFlowFile: problem reading file %s", filename);
	...
	//	if ((int)fread(ptr, sizeof(float), n, stream) != n)
		...
		f_read(&stream, ptr, sizeof(float)*n, &read_len);
		if(read_len != sizeof(float)*n)
			throw CError("ReadFlowFile(%s): file is too short", filename);
    }

//    if (my_getc(&stream) != EOF)
      if (my_getc(&stream) != '\0')
